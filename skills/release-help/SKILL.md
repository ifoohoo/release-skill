...
node "${CLAUDE_PLUGIN_ROOT}/bin/release-skill.mjs" route --root <path> --target-version <version> --json
```
...
- CLI 入口缺失：确认 `${CLAUDE_PLUGIN_ROOT}/bin/release-skill.mjs` 存在；缺失时重新安装插件。
